...
function generateRefreshToken(uid) {
    ...
    const token = jwt.sign({ rGen: Math.random().toString(36).substring(7) }, jwtKey, {
        algorithm: 'HS256'
    });
...
// Validate a refresh token, returning its payload or null
// Potentially give this a larger breadth of return values to differentiate failure types
function verifyRefreshToken(token, uid) {
    if (!token)
        return null;

    const dbToken = refreshToken.findOne({ token: token, user: uid }).exec();

    // This token does not exist in the database so therefore is invalid
    if (!dbToken)
        return null;
